...
import MoldSearch from './MoldSearch'; // MoldSearch 컴포넌트 import
import {
    Paper,
    Button,
    CircularProgress,
...
function MoldList() {
    ...
    const [searchResults, setSearchResults] = useState(null); // 검색 결과를 저장할 상태
    const moldSearchRef = useRef(null); // MoldSearch 컴포넌트에 대한 ref 생성
    ...
    const fetchMolds = useCallback(async () => {
        try {
            ...
            setMolds(data);
            setSearchResults(null); // 전체 목록을 보여줄 때는 검색 결과 초기화
        } catch (err) {
    ...
    const handleSearch = (results) => {
        setSearchResults(results); // MoldSearch 컴포넌트에서 검색 결과를 받아 상태에 저장
    };

    const triggerSearch = () => {
        if (moldSearchRef.current && typeof moldSearchRef.current.handleSubmit === 'function') {
            moldSearchRef.current.handleSubmit(new Event('submit')); // MoldSearch의 handleSubmit 함수를 호출
        }
    };

    // 검색 결과가 있으면 검색 결과를, 아니면 전체 목록을 표시
    const moldsToDisplay = searchResults !== null ? searchResults : molds;
    ...
    return (
        <div>
            ...
            <div style={{ marginBottom: '20px' }}>
                <MoldSearch onSearch={handleSearch} ref={moldSearchRef} /> {/* MoldSearch 컴포넌트에 ref 연결 */}
            </div>

            {searchResults !== null && (
                <Typography variant="h6" style={{ marginTop: '20px' }}>검색 결과</Typography>
            )}

            {isAddingNew && (
                <Paper style={{ padding: '15px', marginBottom: '10px' }}>
                    <Typography variant="h6">새 몰드 추가</Typography>
                    <TextField label="몰드 ID" value={newMoldId} onChange={(e) => setNewMoldId(e.target.value)} fullWidth margin="normal" />
                    <Select label="상태" value={newStatus} onChange={(e) => setNewStatus(e.target.value)} fullWidth margin="normal">
                        <MenuItem value="Received">Received</MenuItem>
                        <MenuItem value="Shipped">Shipped</MenuItem>
                        {/* 필요한 상태 옵션 추가 */}
                    </Select>
                    <TextField label="상태 날짜/시간" type="datetime-local" value={newStatusDate} onChange={(e) => setNewStatusDate(e.target.value)} fullWidth margin="normal" />
                    <Select label="검사 상태" value={newInspectionStatus} onChange={(e) => setNewInspectionStatus(e.target.value)} fullWidth margin="normal">
                        <MenuItem value="WAITING">WAITING</MenuItem>
                        <MenuItem value="PASS">PASS</MenuItem>
                        <MenuItem value="FAIL">FAIL</MenuItem>
                        {/* 필요한 검사 상태 옵션 추가 */}
                    </Select>
                    <TextField label="검사자" value={newInspector} onChange={(e) => setNewInspector(e.target.value)} fullWidth margin="normal" />
                    <TextField label="몰드 카운트" type="number" value={newMoldCount} onChange={(e) => setNewMoldCount(e.target.value)} fullWidth margin="normal" />
                    <Button variant="contained" color="primary" onClick={handleSaveNewMold} style={{ marginRight: '10px' }}>
                        저장
                    </Button>
            ...
            {moldsToDisplay.length > 0 ? (
                isMobile ? (
                    moldsToDisplay.map((mold) => (
                        <Paper key={mold.id} style={{ margin: '10px', padding: '15px' }}>
                            ...
                            <Button size="small" color="secondary" onClick={() => handleDelete(mold.id)} style={{ marginRight: '5px' }}>
                                삭제
                            </Button>
                            ...
                            <TableBody>
                                {moldsToDisplay.map((mold) => (
                                    <StyledTableRow key={mold.id}>
                                        ...
                                        <TableCell>
                                            <Button variant="outlined" color="secondary" onClick={() => handleDelete(mold.id)} size="small">
                                                삭제
                                            </Button>
                                        </TableCell>
                                        <TableCell>
                ...
                !loading && <Typography>몰드 목록이 비어 있습니다.</Typography>
            )}
